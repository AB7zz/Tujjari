...
<?php
      ...
      if(empty($token)){
        ...
        if((ctype_xdigit($token)!==false)){
          $sql = "SELECT * from verifyacc where verifyaccToken = ?";
          ...
          else
          {
            ...
            if(!$row = mysqli_fetch_assoc($result)){
              ...
              $expiredate = $row['verifyaccExpires'];
              if($datenow > $expiredate){
                echo "This form has expired";
              }else{
                $email = $row['verifyaccEmail'];
                $pass = $row['verifyaccPass'];
                $phone = $row['verifyaccPhone'];
                $name = $row['verifyaccName'];
                $insert_cust = $con->prepare("INSERT into customers (customer_name, customer_email, customer_pass, phone) values (?,?,?,?)");
                $insert_cust->bind_param("sssi",$name,$email,$pass,$phone);
                $insert_cust->execute();
                ...
                <a href="https://tujjari.com"><img class="loginlogo" src="images/logo.png" style="height: 65px; width: 160px;" ></a>
                  ...
                  <h3 style="font-size: 20px;" >Your account has been verified successfully!</h3>
